...
class ViewController: NSViewController {
    ...
    override func viewDidAppear() {
        ...
        let currentOcVersion = (shell(launchPath: "/bin/bash", arguments: ["-c", "nvram 4D1FDA02-38C7-4A6A-9CC6-4BCCA8B30102:MacOS-version | awk '{print $2}'"]) ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        if currentOcVersion != "" {
            if !officialOcVersions.contains(currentOcVersion) {
                if supportedOcVersions.contains(currentOcVersion) {
                    alert.messageText = "You are running a prerelease version of MacOS Configurator."
                ...
                if !officialOcVersions.contains(currentOcVersion) {
                    alert.messageText = "You're running a version of MacOS that this app does not currently support"
            ...
            alert.messageText = "Could not get MacOS version!"
    ...
    @objc func onPlistOpen(_ notification: Notification) {
        ...
        self.view.window?.title = "\((path as NSString).lastPathComponent) - MacOS Configurator"
    ...
    @IBAction func helpButtonAction(_ sender: Any) {
        NSWorkspace.shared.open(URL(string: "https://github.com/acidanthera/MacOSPkg/blob/master/Docs/Configuration.pdf")!)
    }
